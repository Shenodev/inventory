...
| 15 | Rate limit | `bootstrap/app.php` + `AppServiceProvider` `RateLimiter::for('login')` 5/15min per email+ip, `api` 60/min per user/ip, `throttle:login`/`throttle:api` on routes, 429 JSON | Shared store via DB cache when scaled (not in-memory) |
